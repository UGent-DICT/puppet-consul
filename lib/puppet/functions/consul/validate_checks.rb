Puppet::Functions.create_function(:'consul::validate_checks') do
  ...
  def validate_checks(obj)
    ...
    when Hash
      if (obj.key?('http') || (obj.key?('script') || obj.key?('args')) || obj.key?('tcp') || obj.key?('grpc')) && !obj.key?('interval')
        raise Puppet::ParseError, 'interval must be defined for tcp, http, grpc and script checks'
      end

      if obj.key?('ttl')
        if (obj.key?('http') || (obj.key?('args') || obj.key?('script')) || obj.key?('tcp') || obj.key?('grpc')) || obj.key?('interval')
          raise Puppet::ParseError, 'script, http, tcp, grpc and interval must not be defined for ttl checks'
        end
      elsif obj.key?('http')
        ...
        if obj.key?('http') || obj.key?('tcp') || obj.key?('args') || obj.key?('script') || obj.key?('interval') || obj.key?('grpc')
          raise Puppet::ParseError, 'alias_service must not define http, tcp, grpc, args, script, or interval'
        end
      else
